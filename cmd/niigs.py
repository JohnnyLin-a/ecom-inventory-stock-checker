...
def main():
    ...
    # Create niggs session and execute
    niigsCaSession = NiigsCa()
    niigsSessionData = niigsCaSession.execute(webEngine)
    webEngine.driver.quit()

    # Create DBEngine and save niigs results
    db = DBEngine()
    result = niigsCaSession.saveData(db, niigsSessionData)
    ...
    diff = niigsCaSession.getDiffFromLast2SuccessfulRuns(db)
    fullInventory = niigsCaSession.getFullInventory(db)
    ...
    discordWebhookDiff = os.getenv("DISCORD_WEBHOOK_NIIGS_DIFF")
    ...
    if len(data['+']) + len(data['-']) != 0:
        discordWebhookFull = os.getenv("DISCORD_WEBHOOK_NIIGS_FULL")
